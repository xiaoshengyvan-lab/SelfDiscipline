...
本应用无需特殊 SDK 即可配合小米超级岛使用（超级岛是通知的展示层，通知到达即以胶囊形式显示在岛上）：

1. **开启上岛**：设置 → 通知与控制中心 → 超级岛 → 打开「通知上岛」，并允许「自律助手」的通知
   （App 内：设置页 → 系统适配卡片 → 超级岛「去设置」按钮可直达通知设置）
2. 以下通知会显示在超级岛：**使用时长提醒**、**专注完成**、**今日任务提醒**（均为高优先级 + 横幅）
3. **能力边界**：小米目前未向第三方开放「实时活动」常驻接口，因此倒计时无法持续驻留岛上实时跳动（仅系统级应用如计时器/通话支持）；本应用的通知为"到达时上岛"，到达后点击即可打开对应页面
