# 自律助手（SelfDiscipline）

一款基于 **Kotlin** 开发的 Android 自律型应用：后台检测手机使用时长，超限时弹出自律提醒，并告诉你**该去做什么**。

## 功能特性

| 需求 | 实现 |
| --- | --- |
| 统计「从锁屏打开后」的使用时长 | **会话计时**：亮屏/息屏/解锁事件驱动，自解锁（亮屏）起累计屏幕使用时间，锁屏后重新开始 |
| 使用时长过多时弹出自律提醒 | 会话使用时长超过阈值后，通过**全屏通知**弹出提醒页；**无需打开 App**（通知栏也带「10分钟后提醒 / 关闭本次提醒」动作） |
| 提醒后的会话规则 | 提醒触发后**冻结计时**：下次锁屏前不再累计；锁屏再解锁后**重新刷新**为 0 并自动恢复提醒 |
| 每日任务刷新 | 已完成任务**保留**，每日 0 点自动重置为未完成；每天按优先级**提醒一次**今日任务（避免每次解锁重复提醒冲突） |
| 提醒设置 | 设置页「提醒设置」**折叠**展示，点击展开阈值/间隔配置 |
| 快速待办 | 首页底部居中双按钮：**＋ 添加任务**（进入添加任务页）与 **☰ 任务清单** |
| 任务清单 | 无勾选框：任务在专注页完成后标记「已完成」，**当天不再弹出提醒**，每日 0 点刷新 |
| 提醒页操作 | 专注页只显示**居中倒计时**：点击卡片开始/暂停（暂停不重置，走完才重置）；「我已完成这个任务」「换一个任务」「关闭本次提醒」 |
| 专注结束提醒 | 倒计时正常走完自动弹出「专注完成」通知并重置 |
| 计时准确性 | 锁屏界面/息屏显示(AOD)/通知点亮不计入；快速锁屏再解锁正确刷新；提醒后冻结、下次解锁重置 |
| 提醒显示该去做什么 | 自动推荐优先级最高的未完成任务，显示名称与预计时长，支持专注倒计时 |
| 可关闭 / 可开启的开关 | 设置页「自律提醒」开关，关闭即停止服务；开机后若开关为开则自动恢复 |
| 自行输入要做的事情 | 任务清单：添加 / 编辑 / 删除 / 标记完成；主页支持**快速添加待办** |
| 自行安排任务时长 | 每个任务可设置预计时长（分钟），提醒页「开始专注」按此时长倒计时 |
| 首页（专注表盘） | 首页只保留**核心圆形表盘**：圆环绿色进度 + 12/3/6/9 极简刻度，内部显示待办名称/倒计时/提示；点击表盘开始/暂停，长按标记完成；下方为权限条、今日统计与底部 ＋/☰ 双按钮 |
| 白色界面 | 强制浅色主题，白色卡片 + 绿色强调色，精简文案 |
| 低耗电 | 事件驱动计时 + 亮屏期间 5 分钟低频检查，不轮询、不查 UsageStats，息屏零主动工作 |
| 手机兼容性 | minSdk 26（Android 8.0+）；适配 Android 13 通知权限、Android 14 前台服务类型、HyperOS/MIUI 与 MagicOS 专属权限引导 |

## 技术栈

- 语言：Kotlin 1.9.24
- UI：Material 3（Material Components 1.12）、ViewBinding
- 数据库：Room 2.6.1（KSP 注解处理）
- 异步：Kotlin 协程 + Flow
- 构建：AGP 8.5.2 / Gradle 8.9 / JDK 17

## 获取 APK：GitHub Actions 云构建（免装 Android Studio）

不想在本机装 Android Studio？工程内置了云构建工作流（`.github/workflows/build.yml`），把工程推到 GitHub 即可在云端自动编译出 APK：

1. 在 GitHub 新建仓库（Public/Private 均可），把 `SelfDiscipline` 工程推送上去；
2. 打开仓库 **Actions** 页面，选择 **Build APK** 工作流 → **Run workflow**（或在 `main`/`master` 分支推送、打 `v*` 标签时自动触发）；
3. 运行成功后，在本次运行底部 **Artifacts** 中下载 `app-debug-apk`；
4. 解压得到 `app-debug.apk`，传到手机安装即可（Debug 包已自动签名，个人自用无限制）。

> 说明：仓库未包含 `gradle-wrapper.jar`（二进制），工作流会自动从 Gradle 官方仓库恢复后再编译，无需手动处理。

## 环境要求

- Android Studio **Hedgehog (2023.1.1)** 或更高版本（自带 JDK 17）
- 一台 Android 8.0+ 的真机（模拟器对使用时长统计支持不完整，建议真机测试）

## 打开与构建

1. 用 Android Studio 打开本目录（`File → Open`，选择 `SelfDiscipline` 文件夹）。
2. 等待首次 Gradle 同步完成（会自动下载 Gradle 8.9 与依赖，需联网）。
3. 若提示缺少 `gradle-wrapper.jar`（命令行构建报错时）：
   - 在 Android Studio 中执行一次 `Sync` 即可自动生成；或
   - 在项目根目录用本机 Gradle 执行 `gradle wrapper` 生成。
4. 连接手机（开启 USB 调试）→ 点击 ▶ 运行。
5. 命令行构建：`./gradlew assembleDebug`（Windows 下 `gradlew.bat assembleDebug`），产物在 `app/build/outputs/apk/debug/`。

> 说明：首次同步需要从 Google Maven / Maven Central 下载依赖，请确保网络可访问 `google()` / `mavenCentral()`。

## 首次使用（3 步）

1. **授予「使用情况访问」权限**：点击主界面权限卡片的「去授权」，在系统设置中为本应用打开开关。
2. **添加任务**：进入「任务清单」，点右下角 `+`，输入任务名称、预计时长、优先级（数字越小越优先）。
3. **打开开关并设置阈值**：回到主界面打开「自律提醒开关」；到「提醒设置」中设置阈值（如 60 分钟）与提醒间隔（如 30 分钟），建议同时开启「忽略电池优化」保证后台稳定。

验证提醒效果：主界面底部有「模拟触发自律提醒（测试）」入口，可随时查看提醒页效果。

## 使用说明

### 主页
- **本次解锁使用**：自解锁/亮屏起累计的屏幕使用时长与阈值进度（锁屏后重新开始）。
- **下一步要做**：自动推荐最优先的未完成任务与预计时长，一键「开始专注」。
- **快速添加待办**：输入即添加（默认 30 分钟），下方显示未完成数量。
- 未授予「使用情况访问」权限时显示引导条，**授权成功后自动隐藏**；右上角齿轮进入设置页。

### 设置页
- **自律提醒开关**：开启后启动后台监控；关闭即停止。
- **提醒设置**：使用时长阈值（5–1440 分钟）、提醒间隔（5–720 分钟）。
- **我的任务清单** / **模拟触发提醒** 入口。
- 权限引导（仅未授权时显示）、电池优化、系统适配（仅小米系显示）。

### 提醒页（全屏弹出）
- 显示本次解锁已使用时长，并推荐「下一个最该做的任务」。
- **开始专注**：按任务设置的预计时长倒计时，进度条实时更新。
- **我已完成** / **换一个任务** / **稍后再说**。

## 权限说明

| 权限 | 用途 | 说明 |
| --- | --- | --- |
| 使用情况访问（PACKAGE_USAGE_STATS） | 读取手机使用时长 | 系统特殊权限，需在系统设置中手动授予 |
| 通知（POST_NOTIFICATIONS，Android 13+） | 提醒弹出 | 首次启动时申请 |
| 前台服务 / specialUse（Android 14+） | 后台持续监控 | 常驻通知可见 |
| 全屏通知（USE_FULL_SCREEN_INTENT） | 提醒页全屏弹出 | 清单声明 |
| 电池优化 | 后台不被系统杀掉 | 可选，建议开启 |
| 开机自启（RECEIVE_BOOT_COMPLETED） | 重启后恢复监控 | 开关为开时生效 |

## HyperOS / MIUI / MagicOS 专属设置

本应用基于标准 Android API，可在各厂商系统上正常运行。但国产 ROM 对后台服务管控较严，为保证**后台监控不被杀掉、提醒页正常弹出**，请在对应系统完成以下设置（App 已内置「系统适配」卡片，自动识别设备并提供一键跳转，见 `util/DeviceCompat.kt`）：

### 荣耀 MagicOS（全功能适配）
1. **应用启动管理**：设置 → 应用 → 应用启动管理 → 自律助手 → 改为**手动管理** → 打开**允许自启动** 与 **允许后台活动**
2. **电池优化**：设置 → 应用 → 应用管理 → 自律助手 → 耗电 → 电池优化 → **不限制**
3. **应用内其他界面（后台弹窗）**：设置 → 应用 → 应用管理 → 自律助手 → 权限 → **应用内其他界面 → 允许**
   （否则使用超限时的全屏提醒可能被系统拦截弹不出来）
4. **灵动胶囊（MagicOS 8+）**：设置 → 通知与控制中心 → 灵动胶囊 → 开启，并允许「自律助手」通知上胶囊
   （提醒通知到达时会以胶囊显示；App 内适配卡「灵动胶囊 · 去设置」可直达本应用通知设置）
5. **通知权限**：保持开启（Android 13+ 首次启动时申请）。
6. **使用情况访问**：设置 → 应用 → 应用管理 → 自律助手 → 权限 → 允许访问使用情况。

### 小米 HyperOS / MIUI
1. **自启动**：设置 → 应用设置 → 应用管理 → 自律助手 → 自启动 → **允许**
2. **后台弹出界面**：设置 → 应用设置 → 应用管理 → 自律助手 → 权限管理 → **后台弹出界面 → 允许**
3. **省电策略**：设置 → 应用设置 → 应用管理 → 自律助手 → 省电策略 → **无限制**
4. **通知权限**：保持开启。
5. **使用情况访问**：设置 → 应用设置 → 授权管理 → 使用情况访问权限 → 允许自律助手。

> 说明：「系统适配」卡片仅在小米系或荣耀系设备上显示，其他品牌自动隐藏；跳转按钮无法直达时会自动回退到应用详情页。

## 动态岛通知（超级岛 / 焦点通知 / 灵动胶囊 / 流体云 / 实况窗）

这些均为**基于通知的灵动岛式展示层**，本应用无需专属 SDK，高优先级提醒通知（使用时长提醒 / 专注完成 / 今日任务提醒）到达时即会在对应区域显示：

| 品牌 | 系统 | 功能名 | 开启路径（参考） |
| --- | --- | --- | --- |
| 小米 | HyperOS | 超级岛 | 设置 → 通知与控制中心 → 超级岛 → 通知上岛 |
| vivo / iQOO | OriginOS | 焦点通知 | 设置 → 通知与状态栏 → 焦点通知 / 应用通知 |
| 荣耀 | MagicOS | 灵动胶囊 | 设置 → 通知 → 灵动胶囊（部分机型：灵动胶囊开关） |
| OPPO / 一加 | ColorOS | 流体云 | 设置 → 通知与状态栏 → 流体云 |
| 华为 | EMUI / HarmonyOS | 实况窗 | 设置 → 通知 → 实况窗 |

App 内：设置页 → 系统适配卡片（按品牌自动显示对应名称）→ 「去设置」按钮直达本应用通知设置页。
**能力边界**：各品牌目前均未向第三方开放「实时活动」常驻接口，倒计时无法持续驻留动态岛上实时跳动；本应用通知为"到达时显示"，点击即可打开对应页面。

参考：[小米官方 Powerkeeper 管控说明](https://dev.mi.com/xiaomihyperos/documentation/detail?pId=1628)、[MIUI→HyperOS 后台服务迁移指南](https://coldfusion-example.blogspot.com/2026/03/migrating-app-background-services-from.html?m=0)

## 省电方案（已实现）

1. **事件驱动计时，不轮询查询**：亮屏 / 息屏 / 解锁广播驱动计时（`SessionUsage`），完全不再查询 `UsageStatsManager`。
2. **亮屏期间 5 分钟低频检查**：仅亮屏时每 5 分钟检查一次阈值并更新通知；协程 `delay` 不会唤醒休眠设备，**息屏后零主动工作**。
3. **无唤醒锁 / 无传感器**：仅用 `elapsedRealtime` 时间戳累计，省去 WakeLock 与传感器开销。
4. **状态持久化**：会话状态存入 SharedPreferences，服务被杀重启可恢复，避免重复计算。
5. **常驻通知 LOW 优先级**：仅在使用时长变化时更新文案，不做多余刷新。

> 说明：为保证可靠性仍需保留前台服务（Android 会因后台限制杀掉无前台服务的应用），但其空转成本可忽略。

## 常见问题

- **使用时长偏少 / 为 0**：未授予「使用情况访问」权限，或刚安装（统计数据需等待系统生成，一般几分钟后开始累计）。
- **提醒不弹出**：检查通知权限是否授予；检查开关是否打开；检查阈值是否已超过；部分国产 ROM 需在系统设置中允许自启动 / 忽略电池优化。
- **后台被系统杀掉**：在「提醒设置」中开启「允许后台运行」（忽略电池优化），部分 ROM 还需在系统「电池 → 应用管理」中设为不限制。
- **想清空当日统计**：时长按自然日 0 点自动重置，无需手动操作。

## 目录结构

```
SelfDiscipline/
├── app/
│   └── src/main/
│       ├── AndroidManifest.xml
│       ├── java/com/selfdiscipline/app/
│       │   ├── MainActivity.kt            # 主界面：开关 / 使用时长 / 入口
│       │   ├── data/                      # Room 任务 + SharedPreferences 设置
│       │   │   ├── Task.kt / TaskDao.kt / TaskRepository.kt / AppDatabase.kt / AppSettings.kt
│       │   ├── service/                   # 后台监控
│       │   │   ├── UsageMonitorService.kt # 前台服务：轮询 + 阈值判定
│       │   │   ├── UsageStatsHelper.kt    # 使用时长统计工具
│       │   │   ├── ReminderNotifier.kt    # 全屏提醒通知
│       │   │   └── BootReceiver.kt        # 开机自启
│       │   ├── reminder/ReminderActivity.kt  # 提醒页：建议任务 + 专注倒计时
│       │   ├── task/                      # 任务清单 / 编辑 / 适配器
│       │   ├── settings/SettingsActivity.kt  # 阈值 / 间隔 / 权限
│       │   └── util/TimeFormat.kt
│       └── res/                           # 布局 / 主题 / 矢量图标
├── build.gradle.kts / settings.gradle.kts / gradle.properties
└── gradlew(.bat) + gradle/wrapper/
```

## 扩展建议

- 每周使用报告（Room 存储每日汇总）。
- 按 App 分类统计（微信/抖音分别统计）。
- 自律失败惩罚机制（如设置“监督伙伴”）。
- 提醒页接系统日历，推荐任务转为日历事件。
